...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            <form action="{{ isset($ticket) ? route('ticket.update', $ticket->id) : route('ticket.store') }}" method="POST">
                @csrf
                @if(isset($ticket))
                    @method('PUT')
                @endif
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="user_id" name="user_id" required>
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', isset($ticket) ? $ticket->user_id : '') == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
